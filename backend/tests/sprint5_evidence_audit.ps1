...
$evidenceDirectory = Join-Path $repositoryRoot "docs/screenshots/sprint-5"
$reportDirectory = Join-Path $repositoryRoot "docs/reports/sprint-5"
$reportPath = Join-Path $reportDirectory "sprint-5-evidence-register.md"
...
if (-not (Test-Path $evidenceDirectory)) {
  throw ("Sprint 5 evidence directory was not found: " + $evidenceDirectory)
}
...
$reportLines.Add("# Sprint 5 Evidence Register")
$reportLines.Add("")
...
$reportLines.Add("")
$reportLines.Add("Security behaviours already demonstrated in earlier sprint evidence are referenced in the Sprint 5 report rather than captured again.")
...
if ($hasErrors) {
  Write-Host "SPRINT 5 EVIDENCE AUDIT FAILED"
  exit 1
}

Write-Host "SPRINT 5 EVIDENCE AUDIT PASSED"
